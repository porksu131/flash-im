 ...
 */
@Service
@RocketMQMessageListener(topic = MqTopicConstant.NOTIFY_DISPATCH_TOPIC,
        consumerGroup = "${rocketmq.consumers.notify-group}",
        messageModel = MessageModel.CLUSTERING)
public class DispatchNotifyConsumer implements RocketMQListener<RocketMqMessage> {
    private static final Logger LOGGER = LoggerFactory.getLogger(DispatchNotifyConsumer.class);
    ...
    public void onMessage(RocketMqMessage message) {
        ...
        dispatchService.dispatchNotify(message);
    }
}
